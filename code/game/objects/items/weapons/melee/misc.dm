...
/obj/item/weapon/melee/balisong
	...
	desc = "A deadly sharp concealable knife, the balisong allows for quick draw and some tricks along the way."
	icon = 'icons/obj/weapons.dmi'
...
/obj/item/weapon/melee/balisong/attack_self(mob/user)
	if(clumsy_check && (user.disabilities & CLUMSY) && prob(50))
		...
		user << "<span class='warning'>You fumble around with the [src], and cut yourself!</span>"

	on = !on
	if(on)
		user << "<span class ='warning'>You flick the [src] open with a flick of your wrist.</span>"
