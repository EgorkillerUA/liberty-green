/obj/item/weapon/gun/portalgun	//-by Deity Link
	...
	lefthand_file = 'icons/mob/inhands/guns_experimental_lefthand.dmi'
	righthand_file = 'icons/mob/inhands/guns_experimental_righthand.dmi'
	recoil = 0
	flags = FPRINT
	w_class = 3.0
	fire_delay = 0
...
/obj/item/weapon/gun/portalgun/examine(mob/user)
	...
	switch(setting)
		if(0)
			user.text2tab("It's current setting is <span style='color: #0066FF;'>blue</span>.")
		if(1)
			user.text2tab("It's current setting is <span style='color: #FF6600;'>red</span>.")
...
/obj/item/weapon/gun/portalgun/process_chamber()
	if(chambered) return 1
	chambered = new/obj/item/projectile/portalgun(src)
	var/obj/item/projectile/portalgun/P = chambered
...
/obj/item/weapon/gun/portalgun/attack_self(mob/user)
	switch(setting)
		if(0)
			...
			user.text2tab("Now set to fire <span style='color: #FF6600;'>red portals</span>.")
		if(1)
			setting = 0
			fire_sound = 'sound/weapons/portalgun_blue.ogg'
			user.text2tab("Now set to fire <span style='color: #0066FF;'>blue portals</span>.")
	update_icon()
